...
<section>
	...
	<div style="width: 50%; float: right;">
		<select name="searchType"
			style="margin: auto; display: inline-block; width: 15%;">
			<option value="n"
				<c:out value="${pageMaker.searchType==null?'selected':'' }"/>>----</option>
			<!-- <option value="n" selected>----</option>
    		<option value="n">----</option> -->
			<option value="t"
				<c:out value="${pageMaker.searchType eq 't'?'selected':'' }"/>>글제목</option>
			<option value="c"
				<c:out value="${pageMaker.searchType eq 'c'?'selected':'' }"/>>글내용</option>
			<option value="w"
				<c:out value="${pageMaker.searchType eq 'w'?'selected':'' }"/>>작성자ID</option>
			<option value="tc"
	...
	<table class='customers'
		style="margin-top: 80px; width: 80%; position: relative;">
		<tr>
		...
		<c:forEach items="${blist}" var="dto">
			<tr>
				<td style="width: 10px">${dto.bid }</td>
						<td>
				<a href="/boardQna/read${pageMaker.makeSearch()}&bid=${dto.bid}"><c:forEach var="indento" begin="1" end="${dto.bindent}" step="1">&ensp;</c:forEach><c:if test="${dto.bindent ne 0}">└</c:if>
				${dto.btitle }</a></td>
	...
	<div class="pagination">
		...
		<button class='writeBtn' style="position: absolute; right: 10%; margin: 10px;">글쓰기</button>
