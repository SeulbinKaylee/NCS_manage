		...
		<c:forEach items="${inScoreList}" var="inScoreList">
			<tr>
				...
				<c:when test="${inScoreList.sum ne '미실시'}">
					<td><a href="/institution/inScoreDetail?sname=${inScoreList.name}&cid=${cid}&ncs_num=${ncs_num}&type=${type}">${inScoreList.name }</a></td>
				</c:when>
				<c:otherwise>
					<td><a href="/include/noScoreDetail">${inScoreList.name }</a></td>
				</c:otherwise>	
				</c:choose>		
